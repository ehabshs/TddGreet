...
describe('Greet function:', () => {
  it('Should greet a name with a simple meeting', () => {
    expect(greet('Bob')).to.be.equal('Hello Bob.');
  });

  it('Should handle nulls by greeting with Hello, my friend', () => {
  ...
  it('Should shout when name is upper case', () => {
    expect(greet('JERRY')).to.be.equal('HELLO JERRY!');
  });

  it('Should handle an array of 2 names', () => {
    expect(greet(['Jill', 'Jane'])).to.be.equal('Hello Jill and Jane.');
  });

  it('Should shout an array of 2 names ', () => {
    expect(greet(['JILL', 'JANE'])).to.be.equal('HELLO JILL AND JANE!');
  });

  it('Should shout an array of 2 names: one uppercase and the other lowercase ', () => {
  ...
  it('Should shout an array of more than 2 names', () => {
    expect(greet(['AMY', 'BRIAN', 'CHARLOTTE'])).to.be.equal('HELLO AMY, BRIAN, AND CHARLOTTE!');
  });

  it('Should handle uppercase and lowercase names (2 names lower, and 1 upper)', () => {
    expect(greet(['Amy', 'BRIAN', 'Charlotte'])).to.be.equal('Hello Amy and Charlotte. AND HELLO BRIAN!');
  });


  it('Should handle uppercase and lowercase names (2 names upper, and 1 lower)', () => {
    expect(greet(['AMY', 'BRIAN', 'Charlotte'])).to.be.equal('Hello Charlotte. AND HELLO AMY AND BRIAN!');
  });

  it('Should handle uppercase and lowercase names (2 names lower, and 1 uppper)', () => {
    expect(greet(['Amy', 'BRIAN', 'Charlotte', 'MADARA'])).to.be.equal('Hello Amy and Charlotte. AND HELLO BRIAN AND MADARA!');
  });

  it('Should handle uppercase and lowercase names (3 names lower, and 1 uppper)', () => {
    expect(greet(['Amy', 'Brian', 'Charlotte', 'MADARA'])).to.be.equal('Hello Amy, Brian, and Charlotte. AND HELLO MADARA!');
  });

  it('Should handle uppercase and lowercase names (3 names upper, and 1 lower)', () => {
    expect(greet(['AMY', 'BRIAN', 'CHARLOTTE', 'madara'])).to.be.equal('Hello madara. AND HELLO AMY, BRIAN, AND CHARLOTTE!');
  });

  it('Should handle uppercase and lowercase names (4 names upper, and 2 lower)', () => {
  ...
  it('Should handle uppercase and lowercase names (4 names lower, and 2 uppper)', () => {
  ...
  it('Should handle string names that contains comma', () => {
    expect(greet(['Bob', 'Charlie, Dianne'])).to.be.equal('Hello Bob, Charlie, and Dianne.');
  });

  it('Should allow intentional commas', () => {
